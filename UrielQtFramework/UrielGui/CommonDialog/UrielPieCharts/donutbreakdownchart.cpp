	...
	: QChart(QChart::ChartTypeCartesian, parent, wFlags)
{
	...
	QFont font(QString::fromUtf8("\346\245\267\344\275\223"), 24);
...
{
	QFont font(QString::fromUtf8("\346\245\267\344\275\223"), 15);
	...
	foreach (QAbstractSeries *series, series()) {
		foreach (QLegendMarker *marker, legend()->markers(series)) {
			...
			if (series == m_mainSeries) {
				...
				pieMarker->setLabel(QStringLiteral("%1 %2(Ԫ)")
									.arg(pieMarker->slice()->label())
									.arg(pieMarker->slice()->value()/*->percentage() * 100*/, 0, 'f', 2));
				pieMarker->setFont(QFont(QString::fromUtf8("\346\245\267\344\275\223"), 15));
			}
		}
	}
}
