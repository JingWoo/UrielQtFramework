...
void QtRPT::printExec(bool maximum, bool direct, QString printerName)
{
    ...
    if (!direct) {
        ...
        //preview.setWindowFlags(Qt::FramelessWindowHint | Qt::WindowSystemMenuHint | Qt::WindowMinimizeButtonHint);
        preview.setStyleSheet("background-color: White");
        //preview.setWindowOpacity(1);
        //preview.setWindowFlags(Qt::FramelessWindowHint);
       // preview.setAttribute(Qt::WA_TranslucentBackground);

        preview.setWindowFlags(Qt::FramelessWindowHint | windowFlags());
        preview.resize(qobject_cast<QWidget *>(this->parent())->geometry().width(), qobject_cast<QWidget *>(this->parent())->geometry().height() - 45);
        preview.move(qobject_cast<QWidget *>(this->parent())->x(), qobject_cast<QWidget *>(this->parent())->y() + 45);
...
//        QRect geom = qobject_cast<QWidget *>(this->parent())->geometry();
//        geom.setTop(30);
//        geom.setLeft(5);
...
//        preview.setGeometry(geom);
        ...
        icon.addPixmap(QPixmap(QString::fromUtf8(":/close.png")), QIcon::Normal, QIcon::On);
        auto exitAct = new QAction(QIcon(":/close.png"),tr("close"),this);
        exitAct->setObjectName("exitAct");
        connect(exitAct, SIGNAL(triggered()), &preview,SLOT(reject()));
