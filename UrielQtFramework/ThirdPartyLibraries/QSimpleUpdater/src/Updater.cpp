...
void Updater::setUpdateAvailable(const bool available)
{
	...
	//    QString text = QStringLiteral("是否现在下载更新?");
	//    QString title = "<h3>"
	//                    + QStringLiteral ("新版本:%1-%2 已发布!")
	//                    .arg (moduleName()).arg(latestVersion())
	//                    + "</h3>";
	...
	//    box.setInformativeText (tr ("无新版本发布"));
	//    box.setText ("<h3>"
	//                 + QStringLiteral("已是最新版本:%1-%2!")
	//				.arg(moduleName()).arg(latestVersion())
	//                 + "</h3>");
	...
	if (option->sVersion != latestVersion()) {
		//if (updateAvailable() && (notifyOnUpdate() || notifyOnFinish())) {
			QString title = QStringLiteral("新版本发布");
			QString Content = QStringLiteral("新版本:%1-%2 已发布!")
				.arg(moduleName()).arg(latestVersion());

			InformationDialog dlg(title, Content, nullptr);
		...
		//else if (notifyOnFinish()) {
			QString title = QStringLiteral("目前无新版本发布");
			QString Content = QStringLiteral("已是最新版本:%1-%2!")
				.arg(moduleName()).arg(latestVersion());
