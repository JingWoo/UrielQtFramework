...
public:
    ...
    static bool parseDir(const QByteArray &buffer, const QString &userName, QUrielUrlInfo *info);

signals:
    void listInfo(const QUrielUrlInfo&);
    void readyRead();
...
static void _q_parseUnixDir(const QStringList &tokens, const QString &userName, QUrielUrlInfo *info)
{
    ...
    permissions |= (p[0] == QLatin1Char('r') ? QUrielUrlInfo::ReadOwner : 0);
    permissions |= (p[1] == QLatin1Char('w') ? QUrielUrlInfo::WriteOwner : 0);
    permissions |= (p[2] == QLatin1Char('x') ? QUrielUrlInfo::ExeOwner : 0);
    permissions |= (p[3] == QLatin1Char('r') ? QUrielUrlInfo::ReadGroup : 0);
    permissions |= (p[4] == QLatin1Char('w') ? QUrielUrlInfo::WriteGroup : 0);
    permissions |= (p[5] == QLatin1Char('x') ? QUrielUrlInfo::ExeGroup : 0);
    permissions |= (p[6] == QLatin1Char('r') ? QUrielUrlInfo::ReadOther : 0);
    permissions |= (p[7] == QLatin1Char('w') ? QUrielUrlInfo::WriteOther : 0);
    permissions |= (p[8] == QLatin1Char('x') ? QUrielUrlInfo::ExeOther : 0);
    info->setPermissions(permissions);

    bool isOwner = info->owner() == userName;
    info->setReadable((permissions & QUrielUrlInfo::ReadOther) || ((permissions & QUrielUrlInfo::ReadOwner) && isOwner));
    info->setWritable((permissions & QUrielUrlInfo::WriteOther) || ((permissions & QUrielUrlInfo::WriteOwner) && isOwner));
}

static void _q_parseDosDir(const QStringList &tokens, const QString &userName, QUrielUrlInfo *info)
{
    ...
    int permissions = QUrielUrlInfo::ReadOwner | QUrielUrlInfo::WriteOwner
                      | QUrielUrlInfo::ReadGroup | QUrielUrlInfo::WriteGroup
                      | QUrielUrlInfo::ReadOther | QUrielUrlInfo::WriteOther;
    QString ext;
    ...
    if (ext == QLatin1String("exe") || ext == QLatin1String("bat") || ext == QLatin1String("com"))
        permissions |= QUrielUrlInfo::ExeOwner | QUrielUrlInfo::ExeGroup | QUrielUrlInfo::ExeOther;
    info->setPermissions(permissions);
...
bool QFtpDTP::parseDir(const QByteArray &buffer, const QString &userName, QUrielUrlInfo *info)
{
    if (buffer.isEmpty())
        return false;
    ...
    : QObject(parent), d(new QFtpPrivate(this))
{
    ...
    connect(&d->pi.dtp, SIGNAL(listInfo(QUrielUrlInfo)),
            SIGNAL(listInfo(QUrielUrlInfo)));
}
...
/*!
    \fn void QFtp::listInfo(const QUrielUrlInfo &i);
